...
const BlogPostTemplate = ({ data, location }) => {
  ...
  const siteTitle = site.siteMetadata?.title || `Hi Prismic`;
    ...
    <Layout location={location} title={siteTitle}>
      ...
      >
        <header>
          ...
          <p>{post.data.publish_date}</p>
        </header>
        <section
          dangerouslySetInnerHTML={{
            __html: post?.data?.description?.html || [],
          }}
...
export const Head = ({ data }) => {
  const { title, excerpt, description } = data?.prismicPost?.data || {};
